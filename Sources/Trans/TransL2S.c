 ...
 *  @note
 */
static inline void Clb_L1SendTimeOut(uint32_t u32CurrSysTick, void *pParam)
{
    STransL2S *pTransL2 = (STransL2S *)pParam;
    ...
    LREP("\r\nL1 TimeOut @ %d", (int)u32CurrSysTick);
...
{
    pTransL2->hL1TimeOutTimer   = (TimerHandle)Timer_Create((FClbTimer)Clb_L1SendTimeOut,pTransL2);
}
/*****************************************************************************/
/** @brief
 *
...
{
    Timer_Stop(pTransL2->hL1TimeOutTimer);
}
/*****************************************************************************/
/** @brief
 *
...
ETransErrorCode TransL2S_Send(STransL2S *pTransL2, uint8_t u8DstAdr, SMem *pMem)
{
    ...
    if(u16DLen != 0)
    {
        pSendFrm[IDX_SFRM_CRCD] = CalCRC8(pData, u16DLen);
...
static EL2RecvAction OnRecvFrame(STransL2S *pTransL2)
{
    ...
    if((u16DLen != 0) && (pRecvFrm[IDX_SFRM_CRCD] != CalCRC8(pRecvData, u16DLen)))
    {
        // Wrong CRC -> Send NACK
