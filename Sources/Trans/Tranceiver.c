...
static void  Clb_UpdateTimer        (uint16_t sTick, void *pClbParam);
...
void Trans_Init(STrans *pTrans,  uint32_t u32UartPort, uint32_t u32BaudRate, uint8_t *pSemaphore)
{
    ...
    TransL2S_Init(&pTrans->sTransL2, RRC_UI_UI_MY_ID, UI_UART_TX_PRIO, UI_UART_RX_PRIO, &pTrans->hSem);
#elif (TRANSL1_VER == TRANSL1_V2)
    TransL2S_Init(&pTrans->sTransL2, RRC_UI_UI_MY_ID, IBC_UART_TX_PRIO, IBC_UART_RX_PRIO, pSemaphore, DMA_TX_TRANSPC);
#endif
    ...
    /*------------------------------------------------------------------------*/
    pTrans->hUpdateTimer = Timer_Create((FClbTimer)Clb_UpdateTimer, pTrans);
    Timer_SetRate(pTrans->hUpdateTimer, TRANS_UPDATE_TIMER_RATE_MS);
    Timer_Start(pTrans->hUpdateTimer);
