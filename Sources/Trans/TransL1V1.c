...
//	};
//
//	XMC_UART_CH_Init(UART[u32UartPort], &uart_config);
//	// set pin for RXD
//	XMC_GPIO_SetMode(UART_RX_PORT[u32UartPort], UART_RX_PIN[u32UartPort],
//			XMC_GPIO_MODE_INPUT_TRISTATE);
//	// set pin for TXD
//	XMC_GPIO_SetMode(UART_TX_PORT[u32UartPort], UART_TX_PIN[u32UartPort],
//			XMC_GPIO_MODE_OUTPUT_PUSH_PULL | UART_TX_AF[u32UartPort]);
//	// set input source pin
//	XMC_UART_CH_SetInputSource(UART[u32UartPort],
//			XMC_UART_CH_INPUT_RXD, UART_RX_INPUT[u32UartPort]);
//	XMC_USIC_CH_EnableEvent(UART[u32UartPort],
//			XMC_USIC_CH_EVENT_STANDARD_RECEIVE);
//	XMC_USIC_CH_SetInterruptNodePointer(UART[u32UartPort],
//			XMC_USIC_CH_INTERRUPT_NODE_POINTER_RECEIVE, UART_ISR_IDX[u32UartPort]);
//	XMC_USIC_CH_EnableEvent(UART[u32UartPort],
//			XMC_USIC_CH_EVENT_ALTERNATIVE_RECEIVE);
//	XMC_USIC_CH_SetInterruptNodePointer(UART[u32UartPort],
//			XMC_USIC_CH_INTERRUPT_NODE_POINTER_ALTERNATE_RECEIVE, UART_ISR_IDX[u32UartPort]);
//
//	XMC_USIC_CH_EnableEvent(UART[u32UartPort], XMC_USIC_CH_EVENT_TRANSMIT_SHIFT);
//	XMC_USIC_CH_SetInterruptNodePointer(UART[u32UartPort],
//			XMC_USIC_CH_INTERRUPT_NODE_POINTER_TRANSMIT_SHIFT, UART_ISR_IDX[u32UartPort]);
//
//	NVIC_SetPriority(UART_IRQn[u32UartPort],u8RxPrio);
//	NVIC_EnableIRQ(UART_IRQn[u32UartPort]);
//
//	XMC_UART_CH_Start(UART[u32UartPort]);

	//LREP("init done uart: 0x%x\r\n", UART[u32UartPort]);
}
...
//        UART_RX_ENTER_CRITICAL();
	...
	//XMC_UART_CH_Transmit(UART[pTransL1->u32UartPort],u8Send);
...
//        Led_Toggle(LED_RADIO);

	/* Push data into FIFO */
	while(FIFO_IsEnablePush(pRecvFF) == TRUE)
	{
		//u8Recv = (uint8_t)XMC_UART_CH_GetReceivedData(UART[pTransL1->u32UartPort]);
